...
개발 서버는 같은 실험실/LAN의 다른 기기에서도 볼 수 있도록 `0.0.0.0`에 바인딩된다. 실행 로그의
`Network` 주소 또는 서버의 실제 LAN 주소(예: `http://10.202.1.12:5173/`)로 접속한다. 주소가
열리지 않으면 서버에서 `hostname -I`로 IP를 다시 확인하고, 5173/TCP 방화벽·VM/컨테이너 포트
포워딩·클라이언트와 서버의 동일 네트워크 여부를 확인한다. Vite 개발 서버는 운영 서버가
아니므로 신뢰할 수 없는 공용 네트워크나 인터넷에 직접 노출하지 않는다.
