...
월드를 폐기하고 이 값으로 동일 레이아웃의 월드를 새로 만들며, 일시정지·고배속 상태도 1×로
정규화한다. 따라서 시나리오 시작 전 배속이나 버튼 클릭 시점이 달라도 초기 사람 배치, 사고 대상,
RMF 로컬 배정과 결과 KPI가 동일하고 발표자가 멈춘 화면이나 16× 화면을 실수로 이어받지 않는다.

`cameraCues[].duration`은 해당 phase 샷을 뒤이은 일반 태스크 `assigned` 카메라로부터 보호하는
wall-clock 시간(초)이다. 실제 인계·조작처럼 더 구체적인 상호작용 샷은 우선할 수 있다. 여러 샷을
순차 편집하는 타임라인이나 사용자 개입 후 자동 재개 정책을 의미하지는 않는다.
