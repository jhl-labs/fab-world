...
- 설비·스토커·안전 장치·구조 기둥은 렌더 치수와 같은 단일 지상 점유 경계를 사용한다. 보행자와
  AGV·IGV·휴머노이드는 8m 공간 해시에서 주변 경계만 조회하고, 0.75~1.8m 전방 swept-circle이
  막히면 개체 id로 고정된 좌우 후보 중 열린 쪽으로 회피한다. 실제 적분 구간도 다시 연속 검사하며,
  동적 개체 간에는 신체 외피 제한 투영을 적용한다. 베이 그래프는 설비 중심이 아니라 로드포트
  접근점에 연결하고 스토커 미션은 캐비닛 중심 대신 시설 측 도킹점에서 끝난다.
- AGV·IGV 초기 위치는 차체 외피끼리 겹치지 않고 휴머노이드 운영 스테이징과도 분리된 도로
  노드에서 결정한다. 가스 경보의 안전 주차 노드도 다른 지상 차량 목표와 2.2m, 휴머노이드
  현재 위치와 3m 이상 떨어진 후보만 예약한다.
- 정지 차량·대기 휴머노이드는 공통 정지 판정으로 traffic 감속, 경로 비용, waypoint 점유 검사에
  동일하게 반영한다. 4초 동안 waypoint 진전이 없으면 정지 차체와 겹치는 graph node를 제외해
  재경로를 찾고, 해당 node가 그래프 절단점이면 연결 경로를 유지한 채 저속 측방 회피로 통과한다.
  집결 슬롯은 정지 차체 외피에서 추가 0.42m 떨어진 위치만 배정하고, 막힌 슬롯 재배정은 이미
  실패한 슬롯을 다시 선택하지 않는다.
...
- 필수 테스트:
  ...
  - 시설 경계: 고정 설비 횡단 0회, 기둥 정면 접근 시 측방 회피, 지상 로봇 외피 중첩 0회
